...
box_pose = [0., 0., np.pi / 2, box_positions[box_pt][0],
            box_positions[box_pt][1], 0.6 + (box_size[2] / 2.0)]
...
hand_pointer = rd.Robot.create_box(box_size, box_pose, "free", 0.1, [
                          0.9, 0.1, 0.1, 1.0], "box_" + str(0))
...
mask_cmd = np.concatenate([np.zeros(17), np.ones(7), np.zeros(4)])
...
for step in range(total_steps):
    if (simu.schedule(simu.control_freq())):
        ...
        fsm()
        arm_tf = robot.body_pose(end_ef)
        hand_pointer.set_base_pose(arm_tf)
        if box_into_basket(
